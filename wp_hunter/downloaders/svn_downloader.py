...
class SVNDownloader:
    ...
    def download_plugin(
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                error_msg = result.stderr.strip() or f"SVN exit code: {result.returncode}"
                return SVNDownloadResult(
                    slug=slug,
                    success=False,
                    error=error_msg
    ...
    def download_many(
        ...
        if not self._check_svn_available():
            if verbose:
                print(f"{Colors.RED}[!] SVN is not available. Please install subversion.{Colors.RESET}")
        ...
        if verbose:
            ...
            print(f"{Colors.BOLD}📥 Downloading {len(slugs)} plugins from SVN{Colors.RESET}")
        ...
        with ThreadPoolExecutor(max_workers=self.workers) as executor:
            ...
            for future in as_completed(future_to_slug):
                ...
                try:
                    ...
                    if verbose:
                        print(f"  [{completed}/{len(slugs)}] {Colors.RED}✗{Colors.RESET} {slug}: {e}")
    ...
    def get_available_versions(self, slug: str) -> List[str]:
        """Get available versions (tags) for a plugin."""
        try:
            svn_url = f"{self.SVN_BASE_URL}/{slug}/tags/"
            result = subprocess.run(
                ["svn", "list", svn_url],
                capture_output=True,
                text=True,
