...
class PluginSyncer:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    with print_lock:
                        print(f"{Colors.YELLOW}[!] Rate limited on page {page}, waiting {wait_time}s...{Colors.RESET}")
                    time.sleep(wait_time)
                    continue
                
                else:
                    with print_lock:
                        print(f"{Colors.RED}[!] API error {response.status_code} on page {page}{Colors.RESET}")
                    return []
                    
            except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e:
                with print_lock:
                    print(f"{Colors.YELLOW}[!] Network error on page {page}: {e}, retrying...{Colors.RESET}")
                time.sleep(2 * (attempt + 1))
                continue
            except Exception as e:
                with print_lock:
                    print(f"{Colors.RED}[!] Unexpected error on page {page}: {e}{Colors.RESET}")
                return []
    ...
    def sync(self, verbose: bool = True) -> SyncProgress:
        ...
        try:
            with ThreadPoolExecutor(max_workers=self.config.workers) as executor:
                ...
                for future in as_completed(future_to_page):
                    ...
                    try:
                        ...
                        if verbose:
                            import sys
                            print(f"  {Colors.GREEN}✓{Colors.RESET} Page {page} done ({plugin_count} plugins) [{completed_pages}/{self.config.pages}]")
                            sys.stdout.flush()
                        ...
                        with print_lock:
                            print(f"{Colors.RED}[!] Error processing page {page}: {e}{Colors.RESET}")
            ...
            self.repository.record_sync_complete(
                ...
                self.progress.plugins_synced
            )
            
        except Exception as e:
            self.progress.error = str(e)
            self.progress.is_running = False
            self.repository.record_sync_complete(sync_id, completed_pages, self.progress.plugins_synced, str(e))
    ...
    def sync_specific_slugs(self, slugs: List[str], verbose: bool = True) -> int:
        ...
        with ThreadPoolExecutor(max_workers=self.config.workers) as executor:
            future_to_slug = {
                executor.submit(self.fetch_plugin_info, slug): slug
                for slug in slugs
            }
