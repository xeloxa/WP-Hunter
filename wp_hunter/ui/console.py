...
def display_plugin_console(idx: int, result: PluginResult) -> None:
    ...
    try:
        ...
        if tested_ver < CURRENT_WP_VERSION - 0.5:
            compat_display = f"{Colors.RED}Outdated (WP {r.tested_wp_version}){Colors.RESET}"
        else:
    ...
    print(f"{Colors.BOLD}{Colors.CYAN}┌── [{idx}] {r.name} {Colors.RESET}(v{r.version})")
    
    print(f"{Colors.CYAN}│{Colors.RESET}   {Colors.BOLD}SCORE:{Colors.RESET} {score_display}  |  {Colors.BOLD}Compatibility:{Colors.RESET} {compat_display}")
    print(f"{Colors.CYAN}│{Colors.RESET}   {Colors.BOLD}Data:{Colors.RESET} {r.installations}+ Installations | {r.days_since_update} days ago")

    dev_type = f"{Colors.YELLOW}Individual/Indie{Colors.RESET}" if not r.author_trusted else f"{Colors.BLUE}Corporate{Colors.RESET}"
    if r.author_trusted: 
    ...
    if r.security_flags:
        print(f"{Colors.CYAN}│{Colors.RESET}   {Colors.RED}{Colors.BOLD}⚠ SECURITY PATCH: {', '.join(r.security_flags).upper()}{Colors.RESET}")
    elif r.feature_flags:
        print(f"{Colors.CYAN}│{Colors.RESET}   {Colors.YELLOW}{Colors.BOLD}★ NEW FEATURE: {', '.join(r.feature_flags).upper()}{Colors.RESET}")

    if r.risk_tags:
        print(f"{Colors.CYAN}│{Colors.RESET}   {Colors.BOLD}Risk Areas:{Colors.RESET} {Colors.ORANGE}{', '.join(list(set(r.risk_tags))[:5]).upper()}{Colors.RESET}")
    ...
    print(f"{Colors.CYAN}│{Colors.RESET}   {Colors.MAGENTA}[Trac Diff]:{Colors.RESET} {r.trac_link}")
    print(f"{Colors.CYAN}│{Colors.RESET}   {Colors.BLUE}[Download]:{Colors.RESET}  {r.download_link}")
    
    print(f"{Colors.CYAN}│{Colors.RESET}   {Colors.GRAY}--- Vulnerability Intel ---{Colors.RESET}")
    print(f"{Colors.CYAN}│{Colors.RESET}   {Colors.RED}[Smart Dork]:{Colors.RESET} {r.google_dork_link}")
    print(f"{Colors.CYAN}│{Colors.RESET}   {Colors.RED}[WPScan]:{Colors.RESET}     {r.wpscan_link}")
    print(f"{Colors.CYAN}│{Colors.RESET}   {Colors.RED}[Patchstack]:{Colors.RESET} {r.patchstack_link}")
    print(f"{Colors.CYAN}│{Colors.RESET}   {Colors.RED}[Wordfence]:{Colors.RESET}  {r.wordfence_link}")
...
def display_theme_console(idx: int, result: Dict[str, Any]) -> None:
    ...
    print(f"{Colors.BOLD}{Colors.MAGENTA}┌── [{idx}] {result.get('name')} {Colors.RESET}(v{result.get('version')})")
    print(f"{Colors.MAGENTA}│{Colors.RESET}   {Colors.BOLD}Risk:{Colors.RESET} {risk_color}{risk_level} ({risk_score}){Colors.RESET}")
    print(f"{Colors.MAGENTA}│{Colors.RESET}   {Colors.BOLD}Downloads:{Colors.RESET} {result.get('downloads', 0):,} | {Colors.BOLD}Updated:{Colors.RESET} {result.get('days_since_update', '?')} days ago")
    print(f"{Colors.MAGENTA}│{Colors.RESET}   {Colors.BOLD}Author:{Colors.RESET} {result.get('author', 'Unknown')}")
...
def print_summary(summary: Dict[str, Any]) -> None:
    ...
    print(f"High Risk Plugins: {Colors.RED}{summary.get('high_risk', 0)}{Colors.RESET}")
    print(f"Abandoned Plugins: {Colors.YELLOW}{summary.get('abandoned', 0)}{Colors.RESET}")
    print(f"User Facing Plugins: {Colors.MAGENTA}{summary.get('user_facing', 0)}{Colors.RESET}")
    print(f"Risky Categories: {Colors.ORANGE}{summary.get('risky_categories', 0)}{Colors.RESET}")
    print(f"Total Analyzed: {Colors.GREEN}{summary.get('total_found', 0)}{Colors.RESET}")
