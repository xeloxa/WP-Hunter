...
from wp_hunter.database.models import get_db_path, ensure_db_dir
...
class PluginMetadataRepository:
    ...
    def get_plugin(self, slug: str, version: Optional[str] = None) -> Optional[Dict[str, Any]]:
        with get_metadata_db(self.db_path) as conn:
            cursor = conn.cursor()
    ...
    def query_plugins(
        ...
        with get_metadata_db(self.db_path) as conn:
            ...
            if search:
                conditions.append("(name LIKE ? OR slug LIKE ? OR short_description LIKE ?)")
            ...
            valid_columns = {"active_installs", "downloaded", "rating", "last_updated", "fetched_at", "name", "slug"}
    ...
    def _row_to_dict(self, row: sqlite3.Row) -> Dict[str, Any]:
        ...
        # Parse JSON fields
        json_fields = ['tags_json', 'sections_json', 'contributors_json', 
                       'ratings_json', 'banners_json', 'icons_json']
